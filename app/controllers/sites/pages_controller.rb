class Sites::PagesController < ApplicationController
  ...
  def sitemap
    ...
    current_site.components.where({name: "menu", publish: true}).each do |component|
      menus_order.merge!(Weby::Components.factory(component).menu_id => Weby::Components.factory(component).sitemap_position.nil? ? "9999" : Weby::Components.factory(component).sitemap_position)
    end
