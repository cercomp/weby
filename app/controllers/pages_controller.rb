class PagesController < ApplicationController
  ...
  def create
    ...
    redirect_to({:site_id => Site.find(params[:page][:site_id]).name, :controller => 'pages'}, :notice => (t"successfully_created"))
  end

  def update
    @page = Page.find(params[:id])
    @page.update_attributes(params[@page.class.name.underscore])
    redirect_to({:site_id => @page.sites[0].name, :controller => 'pages'}, :notice => (t"successfully_updated"))
  end
