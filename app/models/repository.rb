...
# Metodo para incluir a url do arquivo no json
  ...
  def as_json(options = {})
    json = super(options)
    json[:o_path] = self.archive.url(:o)
    json[:l_path] = self.archive.url(:l)
    json[:m_path] = self.archive.url(:m)
    json[:i_path] = self.archive.url(:i)
    json[:t_path] = self.archive.url(:t)
    json
  end
