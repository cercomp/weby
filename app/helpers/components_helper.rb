    ...
    components_html << "class='component component-#{ compo.name } #{'disabled' unless component_is_available(compo.name)} #{compo.publish ? "" : "deactivated"}' data-place='#{compo.place_holder}'>
      <div>
        ...
        <div class='pull-right'>
          ...
          #{ link_to t("+"), new_site_admin_component_path(placeholder: compo.id), class: "btn btn-success btn-sm", title: t(".new_component") if compo.name.to_s == "components_group" and check_permission(Sites::Admin::ComponentsController, [:new]) and !leftout }
        </div>
