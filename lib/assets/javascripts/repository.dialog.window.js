...
WEBY.RepositoryDialog = function(){
  ...
  var performSearch = function(){
    ...
    $.get(form.attr('action'), form.serialize(), function(data){
      ...
      if(data.repositories.length == 0 && data.num_pages == 0){
        form.next('ul').append('<div class="alert">'+form.data('noresult')+'</div>');
      }else{
        $(data.repositories).each(function (index, node) {
          var repository = new WEBY.Repository(node);
          var checked = isSelected(repository);
          form.next('ul').append(WEBY.Repository.ItemTemplate(repository, settings.field_name || 'repository', settings.multiple, checked));
  ...
  form.next('ul').on('change', 'input', function () {
    var input = $(this);
    if (input.is(':checked')){
