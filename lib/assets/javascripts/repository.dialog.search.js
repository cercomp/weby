   ...
   var appendResult = function () {
      ...
      objectGet.success(function(data){
         ...
         if(data.repositories.length == 0 && data.num_pages == 0){
             includeOnForm.append('<div class="alert">'+form.data('noresult')+'</div>');
         }else{
   ...
   var include = function () {
      try {
         if(dialog.multiple) {
            ...
            $(dialog.link_obj).removeClass("without-image");
            $("#"+dialog.placeName).html(item).siblings(".close").show();
         }
      } catch (error) {
...
function repositorySearch (link) {
   if(WEBY.Repository.repositorySearch) {
      ...
      $('a[data-toggle="pill"]').on('shown', function (e) {
          //console.log(e);
          if(e.target.hash == '#tab_repository'){
              $('#repository-include-link').show();
          }else{
              $('#repository-include-link').hide();
          }
      });
   }
}
