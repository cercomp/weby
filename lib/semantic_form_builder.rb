class SemanticFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def submit_and_cancel(submit_name, cancel_name, options = {})
    ...
    cancel_button = @template.submit_tag(cancel_name, options.merge( :onClick => "this.form.reset(); return false;", :id => "btnCancel"))
  ...
  def radio_button_group(method, values, will, options = {})
    ...
    values.each do |value|
      ...
      selections << boolean_field_wrapper(radio_button, "#{@object_name}_#{method.to_s}", tag_value, value_text, help, br)
  ...
  def check_box_group(method, values, will, options = {})
    selections = []
    values.each do |value|
      if value.is_a?(Hash)
    ...
    field_name, label, options = field_settings(method, options)
    semantic_group("check-box", field_name, label, selections, will, options)    
  end
end
