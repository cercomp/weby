...
"
  ...
  def migrate_this_usuarios
    ...
    select_usuarios = "SELECT * FROM usuarios WHERE status=false #{@limit}"
  ...
  def migrate_this_sites
    ...
    select_sites = "SELECT * FROM sites #{@limit}"
    puts "\t#{select_sites}\n"
    ...
    sites_this.each do |s_this|
      ...
      select_menu_d = "SELECT * FROM menu_direito WHERE site_id='#{site[0]['id']}' AND item_pai=0 #{@limit}"
      puts "\t\t\t#{select_menu_d}\n"
      menus_this_d = @con_this.exec(select_menu_d)
      # Laço this.menu_direito
      menus_this_d.each do |m_d|
        insert_menu = "INSERT INTO menus (title,link) VALUES ('#{m_d['texto_item']}','#{m_d['url']}') RETURNING id"
        puts "\t\t\t\t#{insert_menu}\n"
        menu_d = @con_weby.exec(insert_menu)
        @con_weby.exec("INSERT INTO sites_menus(site_id,menu_id,parent_id,side) VALUES ('#{site[0]['id']}','#{menu_d[0]['id']}','#{m_d['item_pai']}','auxiliary')")
      end
      #menus_this_e = @con_this.exec("SELECT * FROM menu_esquerdo WHERE site_id='#{site[0]['id']}' AND item_pai=0 #{@limit}")
      #menus_this_s = @con_this.exec("SELECT * FROM menu_superior WHERE site_id='#{site[0]['id']}' AND item_pai=0 #{@limit}")
      #menus_this_i = @con_this.exec("SELECT * FROM menu_inferior WHERE site_id='#{site[0]['id']}' AND item_pai=0 #{@limit}")
      menus_this_d.clear()
      #menus_this_e.clear()
      #menus_this_s.clear()
      #menus_this_i.clear()
