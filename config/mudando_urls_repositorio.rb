...
#    obj.send(field).gsub(/\/uploads\/([0-9]+)/){|x| "/up/#{obj.site_id}"}
    obj.send(field).gsub(/\/uploads\/([0-9]+)\/(original|little|thumb|medium|mini)_/){|x| "/up/#{obj.site_id}/#{dic[$2.to_sym]}/"}
  end
end

dic = {original: 'o', little: 'l', medium: 'm', mini: 'i', thumb: 't'}
Component.where(name: ['image','front_news','text','blank','body']).each do |comp|
