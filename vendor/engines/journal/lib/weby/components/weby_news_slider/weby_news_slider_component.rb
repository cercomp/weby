class WebyNewsSliderComponent < Component
  ...
  def news(site)
    news_category.blank? ?
      site.news_sites.where(site_id: site.id).available_fronts.where('repository_id is not null').order('position desc').limit(quantity) :
        site.news_sites.where(site_id: site.id).available.tagged_with(news_category.to_s.mb_chars.downcase.to_s).where('repository_id is not null').order('position desc')
  end
